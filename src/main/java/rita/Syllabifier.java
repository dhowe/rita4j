...
public class Syllabifier {
	...
	static String stringify(String[][] syllables) {
	...
	static String fromPhones(String[] ltsPhones) {
		/*
		 * boolean dbug; int none; String[] internuclei; String[] syllables; // returned
		 * data structure String[] sylls = ltsPhones.split("-");
		 * 
		 * for (int i = 0; i < sylls.length; i++) {
		 * 
		 * String phoneme = sylls[i].trim(); int stress = none;
		 * 
		 * if (phoneme.length() == 0) continue;
		 * 
		 * if (Util.isNum(Util.last(phoneme))) { stress = parseInt(Util.last(phoneme));
		 * phoneme = phoneme.substring(0, phoneme.length - 1); }
		 * 
		 * if (dbug) log(i + ")" + phoneme + " stress=" + stress + " inter=" +
		 * internuclei.join(":"));
		 * 
		 * if (Syllabifier.Phones.vowels.includes(phoneme)) {
		 * 
		 * // Split the consonants seen since the last nucleus into coda and onset. let
		 * coda = none, onset = none;
		 * 
		 * // Make the largest onset we can. The "split" variable marks the break point.
		 * for (int split = 0; split < internuclei.length + 1; split++) {
		 * 
		 * coda = internuclei.slice(0, split); onset = internuclei.slice(split,
		 * internuclei.length);
		 * 
		 * if (dbug) log("  " + split + ") onset=" + onset.join(":") + "  coda=" +
		 * coda.join(":") + "  inter=" + internuclei.join(":"));
		 * 
		 * // If we are looking at a valid onset, or if we"re at the start of the word
		 * // (in which case an invalid onset is better than a coda that doesn"t follow
		 * // a nucleus), or if we"ve gone through all of the onsets and we didn"t find
		 * // any that are valid, then split the nonvowels we"ve seen at this location.
		 * boolean bool = Syllabifier.Phones.onsets.includes(onset.join(" ")); if (bool
		 * || syllables.length === 0 || onset.length === 0) { if (dbug) log("  break " +
		 * phoneme); break; } }
		 * 
		 * // Tack the coda onto the coda of the last syllable. // Can"t do it if this
		 * is the first syllable. if (syllables.length > 0) {
		 * Util.extend(syllables[syllables.length - 1][3], coda); if (dbug)
		 * log("  tack: " + coda + " -> len=" + syllables[syllables.length -
		 * 1][3].length + " [" + syllables[syllables.length - 1][3] + "]"); }
		 * 
		 * // Make a new syllable out of the onset and nucleus. let toPush = [[stress],
		 * onset, [phoneme], []]; syllables.push(toPush);
		 * 
		 * // At this point we"ve processed the internuclei list. internuclei = []; }
		 * else if (!(Syllabifier.Phones.consonants.includes(phoneme)) && phoneme !=
		 * " ") { throw Error("Invalid phoneme: " + phoneme); } else { // a consonant
		 * internuclei.push(phoneme); } }
		 * 
		 * // Done looping through phonemes. We may have consonants left at the end. //
		 * We may have even not found a nucleus. if (internuclei.length > 0) { if
		 * (syllables.length === 0) { syllables.push([[none], internuclei, [], []]); }
		 * else { Util.extend(syllables[syllables.length - 1][3], internuclei); } }
		 * 
